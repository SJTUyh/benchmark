...
class TestHarborTaskResumeJob(unittest.TestCase):
    ...
    @mock.patch('ais_bench.benchmark.tasks.custom_tasks.harbor_task.AISLogger')
    def test_resume_job(self, mock_ais_logger):
        """Test _resume_job restores existing job"""
        mock_logger = mock.MagicMock()
        mock_ais_logger.return_value = mock_logger
        ...
        existing_job_dir = os.path.join(self.temp_dir, "existing_job")
        os.makedirs(existing_job_dir, exist_ok=True)
        config_path = os.path.join(existing_job_dir, "config.json")
        with open(config_path, 'w') as f:
            json.dump({"job_name": "details", "jobs_dir": self.temp_dir}, f)
